...
from experiments.exp_ETTh_gen import Exp_ETTh
...
### -------  dataset settings --------------
parser.add_argument('--data', type=str, required=False, default='ETTh1', help='name of dataset')
...
parser.add_argument('--multiplier', type=str)
parser.add_argument('--syn_model_name', type=str, default='TaskGAN')
parser.add_argument('--gen_hidden', type=float, default=0.25)
parser.add_argument('--gen_layers', type=int, default=5)
...
data_parser = {
    ...
    'us_births': {'data': 'us_births.csv', 'T': 'T1', 'M': [1, 1, 1], 'S': [1, 1, 1], 'MS': [1, 1, 1]},
}
...
if args.evaluate:
    ...
    if args.itr:
        ...
        os.makedirs('event/run_ETTh/{}/{}'.format(args.model_name, args.syn_model_name), exist_ok=True)
        with open(os.path.join('event/run_ETTh/{}/{}'.format(args.model_name, args.syn_model_name), 'result.txt'), 'w') as f:
            f.write('Final mean normed mse:{:.4f},mae:{:.4f},denormed mse:{:.4f},mae:{:.4f}'.format(mse, mae, mses, maes))
